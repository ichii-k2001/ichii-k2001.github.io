---
layout: default
title: Ichii-K Pages
---

# こんにちは、ichii-kです。

クラウドエンジニアとして働いています。  
主にインフラ（クラウド）領域の要件定義～運用・保守に携わっています。

---

## 経歴

### 2025年
- クラウドエンジニアとしてSES企業に入社
- 開発チームとして事前検討～リリースまでを担当

### 2023年〜2025年
- 東京の受託開発企業にて、小～中規模システムの設計・構築・運用を担当
- 主にAWSインフラ構築や、VMware環境の保守運用を経験

### 2021年〜2023年
- 九州の専門学校 クラウドコンピューティング科でITの基礎を学習
- AWS Academy カリキュラムを通してクラウド技術を学習

---

## スキル・関心領域

- AWS（EC2, VPC, ECS, ELB, CloudFront, RDS などの主要サービス）
- Azure（運用・保守レベル）
- GCP（運用・保守レベル）
- IaC（Terraform, AWS CDK 勉強中）
- クラウドマイグレーション
- Windows（ADサーバー移行, WSUS, Windows Upgrade）
- Linux（運用・保守でのコマンド実行レベル）
- GitHub Actions によるCI/CD
- Databricks
- Python, Java

---

## ブログ・アウトプット

- [Note](https://note.com/ichii_k){:target="_blank"}
- [X（旧Twitter）](https://x.com/ichii_k2001){:target="_blank"}
- [GitHub](https://github.com/ichii-k2001){:target="_blank"}

---

## その他

- [仕事の延長線上](./仕事の延長線上/)
- [雑多](./雑多/)

---

ご訪問ありがとうございます！

[↑ ページの先頭へ戻る](#top)
